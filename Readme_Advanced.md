...
In RTX compatibility mode, if the driver's capability block reports `DLSSNR.Available = 0`, the wrapper loads its own `nvngx.dll_dlssnr.dll` forwarder and calls the neural snippet's D3D12 exports directly. The forwarder name contains `nvngx.dll` because the 310.8 neural snippet validates the module that called it before accepting direct calls. The driver NGX core remains initialized and supplies the capability parameter block; super resolution continues through the normal core path.

Because the NGX loader picks these files up by name from a folder anyone can write to, this tool normally checks
every copy of them it finds in either folder before the loader gets there, whenever NGX is started at all and whether or not the session will use that model. In this fork, modified-DLSSNR compatibility is enabled by default; `--allow-modified-dlssnr off` restores strict verification. When compatibility is enabled, Authenticode verification is skipped only for `nvngx_dlssnr.dll`; all other local NVIDIA DLLs keep the checks below. In the normal mode, Windows must accept every Authenticode signature the file carries (a driver file often carries NVIDIA's and Microsoft's), and one of the signers
...
- A DLSS 5 neural model, `nvngx_dlssnr.dll`, next to `FullScreenWrapperForDLSS5.exe` or in the folder given by
  `--ngx-path`. The upstream-style strict path requires NVIDIA's signed model. This fork defaults to RTX 40 compatibility and accepts a compatible modified model unless `--allow-modified-dlssnr off` is explicitly supplied. NGX looks for feature DLLs in the application folder and the listed paths, the way
